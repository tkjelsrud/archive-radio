...
- **Default year tag:** the file's own `modified_time` — its actual creation/last-touched date — not the project folder it happens to sit in. Originally speced the other way around (batch year from the folder, since cloud-reported mtime fidelity was unverified), but real data overturned that: mtime survives the Jottacloud round trip fine, and a folder's year can genuinely be wrong for a specific file (e.g. an older sample/stem reused inside a later project's folder). Falls back to the cloud project-folder year (§5a) only if `modified_time` is missing.
